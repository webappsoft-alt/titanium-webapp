...
const ImportR27Margin = ({ handleR27Margin }) => {
  ...
  const rangeFormat = (totalRange) => {
    let range = {
      start: 0,
      end: 0,
      infinity: false,
    };
    if (totalRange[totalRange.length - 1] === "+") {
      range.start = Number(totalRange.split("+")[0].trim());
      range.infinity = true;
    } else {
      let rangeArray = totalRange.split("-");
      range.start = Number(rangeArray[0].trim());
      range.end = Number(rangeArray[1].trim());
    }
    return range;
  };

  const marginCodeJson = (data) => {
    let marginCode = [];
    Object.entries(data).forEach(([key, value]) => {
      if (key === "Margin Code") return;
      marginCode.push({
        label: key,
        value: value,
      });
  ...
  const validateAndTransformData = (data) => {
    ...
    data.forEach((row) => {
      let structuredItem = {};
      structuredItem = {
        range: rangeFormat(row["Margin Code"]),
        marginCode: marginCodeJson(row),
      };
  ...
  const onSubmit = async (data) => {
    ...
    setIsSubmiting(true);
    await post("r27-margin/create", { marginData: structuredData })
      .then((result) => {
        if (result.success) {
  ...
  return (
    <>
      <Dialog open={isOpen} onClose={closeModal}>
        <DialogContent
          className="w-full max-w-lg transform overflow-hidden rounded-2xl bg-white p-6 text-left align-middle shadow-xl"
          closeButton={true}
          onClose={closeModal}
        >
          <DialogHeader>
          ...
          <form className="mt-4 space-y-4" onSubmit={handleSubmit(onSubmit)}>
            <div className="">
              ...
              >
                ...
                {isLoading ? <Spinner /> : "Import R27 Margin Data File"}
                <input
                  type="file"
                  ref={fileInputRef}
                  hidden
              ...
              {file?.length > 0 && productFile && (
                <div className="">
                  <Link href={productFile} target="_blank">
                    <FormFeedback style={{ fontSize: 16 }}>
                      {file[0]?.name}
                    </FormFeedback>
                  </Link>
                </div>
...
export default ImportR27Margin;
