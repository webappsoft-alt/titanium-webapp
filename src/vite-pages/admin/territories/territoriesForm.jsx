...
export function TerritoriesForm({ rowData }) {
  ...
  const usCountry =
    countriesList?.find((item) => item?.iso === "US")?._id || "";
  const [sorted, setSorted] = useState({
    statesL: [],
    countriesL: [],
  });

  const sortedStates = statesList
    ? [...statesList].sort((a, b) => {
  ...
  const onSubmit = async (data) => {
    ...
    const finalData = {
      ...
      states:
        stateData?.length > 0
          ? stateData?.map((item) => ({ ...item, stateID: item?._id }))
          : [],
      countries:
        countryData?.length > 0
          ? countryData?.map((item) => ({ ...item, countryID: item?._id }))
          : [],
    };
    if (rowData) {
  ...
  const handleSetUserData = (data) => {
    ...
    setValue(
      "states",
      data?.states?.length > 0 ? data?.states?.map((item) => item?._id) : []
    );
    setValue(
      "countries",
      data?.countries?.length > 0
        ? data?.countries?.map((item) => item?._id)
        : []
    );
        ...
        <form className="space-y-3" onSubmit={handleSubmit(onSubmit)}>
          <div className="grid grid-cols-2 gap-x-3 gap-y-3 max-sm:grid-cols-1">
                    ...
                    <Controller
                      ...
                      render={({ field }) => {
                        const Row = ({ index, style }) => {
                          const item =
                            sorted.statesL.length > 0
                              ? sorted.statesL[index]
                              : sortedStates[index];
            ...
            <div>
              <Button
                type="button"
                onClick={() => setIsCountries(!isCountries)}
                variant="outline"
              >
                  ...
                  <div className="pt-3">
                    <Controller
                      ...
                      render={({ field }) => {
                        const Row = ({ index, style }) => {
                          const item =
                            sorted.countriesL.length > 0
                              ? sorted.countriesL[index]
                              : countriesList[index];
                          const isChecked = field.value.includes(item?._id);
          ...
          <div className="flex pt-3">
            <Button type="submit" className="" size="sm" disabled={isLoading}>
              {isLoading ? (
